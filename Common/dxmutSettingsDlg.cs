...
namespace Microsoft.Samples.DirectX.UtilityToolkit
{
    ...
    public class SettingsDialog
    {
        ...
        {
            parent = sample;
            windowWidth = Framework.DefaultSizeWidth;
            windowHeight = Framework.DefaultSizeHeight;
            CreateControls();
        }
        ...
        private void CreateControls()
        {
            ...
            var e = dialog.GetDefaultElement(ControlType.StaticText, 0);
            e.textFormat = DrawTextFormat.VerticalCenter | DrawTextFormat.Right;

            // Title
            var title = dialog.AddStatic((int) SettingsDialogControlIds.Static, "Direct3D Settings", 10, 5, 400, 50);
            e = title[0];
            e.FontIndex = 1;
            ...
            // Windowed / Fullscreen
            windowedButton = dialog.AddRadioButton((int) SettingsDialogControlIds.Windowed,
                (int) SettingsDialogControlIds.RadioButtonGroup,
                "Windowed", 240, 105, 300, 16, false);
            clipBox = dialog.AddCheckBox((int) SettingsDialogControlIds.DeviceClip,
                "Clip to device when window spans across multiple monitors",
                250, 126, 400, 16, false);
            fullscreenButton = dialog.AddRadioButton((int) SettingsDialogControlIds.Fullscreen,
                (int) SettingsDialogControlIds.RadioButtonGroup, "Full Screen",
            ...
            // Resolution
            resolutionStatic = dialog.AddStatic((int) SettingsDialogControlIds.ResolutionLabel, "Resolution", 10, 205,
                180, 23);
            resolution = dialog.AddComboBox((int) SettingsDialogControlIds.Resolution, 200, 205, 300, 23);
            resolution.SetDropHeight(106);

            // Refresh Rate
            refreshStatic = dialog.AddStatic((int) SettingsDialogControlIds.RefreshRateLabel, "Refresh Rate", 10, 230,
                180, 23);
            ...
            multiSampleTypeCombo =
                dialog.AddComboBox((int) SettingsDialogControlIds.MultisampleType, 200, 315, 300, 23);

            // Multisample Quality
            dialog.AddStatic((int) SettingsDialogControlIds.Static, "Multisample Quality", 10, 340, 180, 23);
            multiSampleQualityCombo =
                dialog.AddComboBox((int) SettingsDialogControlIds.MultisampleQuality, 200, 340, 300, 23);
            ...
            var okButton = dialog.AddButton((int) SettingsDialogControlIds.OK, "OK", 230, 435, 73, 31);
            var cancelButton =
                dialog.AddButton((int) SettingsDialogControlIds.Cancel, "Cancel", 315, 435, 73, 31, 0, true);
        ...
        public void Refresh()
        {
            ...
            var client = parent.WindowClientRectangle;
            ...
            AddResolution((short) globalSettings.presentParams.BackBufferWidth,
                (short) globalSettings.presentParams.BackBufferHeight);
            ...
            if (!backBufferCombo.ContainsItem(globalSettings.presentParams.BackBufferFormat.ToString()))
                backBufferCombo.AddItem(globalSettings.presentParams.BackBufferFormat.ToString(),
                    globalSettings.presentParams.BackBufferFormat);

            if (!depthStencilCombo.ContainsItem(globalSettings.presentParams.AutoDepthStencilFormat.ToString()))
                depthStencilCombo.AddItem(globalSettings.presentParams.AutoDepthStencilFormat.ToString(),
                    globalSettings.presentParams.AutoDepthStencilFormat);

            if (!multiSampleTypeCombo.ContainsItem(globalSettings.presentParams.MultiSample.ToString()))
                multiSampleTypeCombo.AddItem(globalSettings.presentParams.MultiSample.ToString(),
                    globalSettings.presentParams.MultiSample);

            if (!multiSampleQualityCombo.ContainsItem(globalSettings.presentParams.MultiSampleQuality.ToString()))
                multiSampleQualityCombo.AddItem(globalSettings.presentParams.MultiSampleQuality.ToString(),
                    globalSettings.presentParams.MultiSampleQuality);

            if (!presentCombo.ContainsItem(globalSettings.presentParams.PresentationInterval.ToString()))
                presentCombo.AddItem(globalSettings.presentParams.PresentationInterval.ToString(),
                    globalSettings.presentParams.PresentationInterval);
            ...
            var adapterInfoList = Enumeration.AdapterInformationList;
            ...
            for (var iAdapter = 0; iAdapter < adapterInfoList.Count; iAdapter++)
            {
                var adapterInfo = adapterInfoList[iAdapter] as EnumAdapterInformation;
        ...
        private readonly Framework parent; // Parent framework for this dialog
        ...
        public void OnCreateDevice(Device d)
        {
            ...
            resolution.Changed += OnResolutionChanged;
            adapterCombo.Changed += OnAdapterChanged;
            deviceCombo.Changed += OnDeviceChanged;
            adapterFormatCombo.Changed += OnAdapterFormatChange;
            refreshCombo.Changed += OnRefreshRateChanged;
            backBufferCombo.Changed += OnBackBufferChanged;
            depthStencilCombo.Changed += OnDepthStencilChanged;
            multiSampleTypeCombo.Changed += OnMultisampleTypeChanged;
            multiSampleQualityCombo.Changed += OnMultisampleQualityChanged;
            vertexCombo.Changed += OnVertexProcessingChanged;
            presentCombo.Changed += OnPresentIntervalChanged;
            clipBox.Changed += OnClipWindowChanged;
            windowedButton.Changed += OnWindowedFullscreenChanged;
            fullscreenButton.Changed += OnWindowedFullscreenChanged;
        }
        ...
        {
            var itemText = string.Format("{0} by {1}", width, height);
            // Store the resolution in a single variable
            var resolutionData = NativeMethods.MakeUInt32(width, height);
        ...
        {
            var itemText = rate == 0 ? "Default Rate" : string.Format("{0} Hz", rate);
            // Add this item
        ...
        {
            var itemText = "Unknown vertex processing type";
            switch (flags)
            {
                case CreateFlags.PureDevice:
                    itemText = "Pure hardware vertex processing";
                    break;
                case CreateFlags.HardwareVertexProcessing:
                    itemText = "Hardware vertex processing";
                    break;
                case CreateFlags.SoftwareVertexProcessing:
                    itemText = "Software vertex processing";
                    break;
                case CreateFlags.MixedVertexProcessing:
                    itemText = "Mixed vertex processing";
                    break;
            }
        ...
        private void OnResolutionChanged(object sender, EventArgs e)
        {
            ...
            // Set the resolution
            var data = (uint) cb.GetSelectedData();
        ...
        private void OnDeviceChanged(object sender, EventArgs e)
        {
            ...
            var hasWindowCombo = false;
            var hasFullscreen = false;

            var edi = GetCurrentDeviceInfo();
        ...
        private void OnAdapterFormatChange(object sender, EventArgs e)
        {
            ...
            var currentResolution = NativeMethods.MakeUInt32(
                (short) globalSettings.presentParams.BackBufferWidth,
                (short) globalSettings.presentParams.BackBufferHeight);
            ...
            var edi = GetCurrentDeviceInfo();
            var hasWindowedBackBuffer = false;
            var isWindowed = windowedButton.IsChecked;
        ...
        private void OnBackBufferChanged(object sender, EventArgs e)
        {
            ...
            var isAllowSoftware = Enumeration.IsSoftwareVertexProcessingPossible;
            var isAllowHardware = Enumeration.IsHardwareVertexProcessingPossible;
            var isAllowPure = Enumeration.IsPureHardwareVertexProcessingPossible;
            var isAllowMixed = Enumeration.IsMixedVertexProcessingPossible;
        ...
        private void OnDepthStencilChanged(object sender, EventArgs e)
        {
            ...
            var combo = GetCurrentDeviceSettingsCombo();
        ...
        private void OnMultisampleTypeChanged(object sender, EventArgs e)
        {
            ...
            var maxQuality = 0;
            for (var i = 0; i < combo.multiSampleTypeList.Count; i++)
            {
                MultiSampleType msType = (MultiSampleType) combo.multiSampleTypeList[i];
                if (msType == mst) maxQuality = (int) combo.multiSampleQualityList[i];
            }
        ...
        private void OnWindowedFullscreenChanged(object sender, EventArgs e)
        {
            ...
            var deviceClip = (globalSettings.presentParams.PresentFlag & PresentFlag.DeviceClip) != 0;
